...
# Install
#######################
mkdir /opt/medusa && sudo chown $username:$username /opt/medusa
git clone https://github.com/pymedusa/Medusa.git /opt/medusa

# Run Medusa for the first time to create default config files
...
sed -i "s/^root_dirs =.*/root_dirs = 0|\/home\/$username\/$overlayfuse\/tv/g" /opt/medusa/config.ini
sed -i "s|naming_pattern =.*|naming_pattern = Season %0S\\\%S_N-S%0SE%0E-%E_N-%Q_N|g" /opt/medusa/config.ini

sed -i "s/^web_username =.*/web_username = $username/g" /opt/medusa/config.ini
sed -i "s/^web_password =.*/web_password = $passwd/g" /opt/medusa/config.ini

sed -i "s/^nzbget_username =.*/nzbget_username = $username/g" /opt/medusa/config.ini
sed -i "s/^nzbget_password =.*/nzbget_password = $passwd/g" /opt/medusa/config.ini
sed -i "s/^nzbget_host =.*/nzbget_host = localhost:6789/g" /opt/medusa/config.ini

sed -i "s/^use_nzbs =.*/use_nzbs = 1/g" /opt/medusa/config.ini
sed -i "s/^nzb_method =.*/nzb_method = nzbget/g" /opt/medusa/config.ini

sed -i "s/^opensubtitles_password =.*/opensubtitles_password = $openSubtitlesPassword/g" /opt/medusa/config.ini
sed -i "s/^opensubtitles_username =.*/opensubtitles_username = $openSubtitlesUsername/g" /opt/medusa/config.ini
sed -i "s/^subtitles_languages =.*/subtitles_languages = $openSubtitlesLang/g" /opt/medusa/config.ini

sed -i 's/^SUBTITLES_SERVICES_LIST =.*/SUBTITLES_SERVICES_LIST = "opensubtitles,addic7ed,legendastv,shooter,subscenter,thesubdb,tvsubtitles"/g' /opt/medusa/config.ini
sed -i "s/^use_subtitles =.*/use_subtitles = 1/g" /opt/medusa/config.ini
sed -i 's/^SUBTITLES_SERVICES_ENABLED =.*/SUBTITLES_SERVICES_ENABLED = 1|0|0|0|0|0|0|0|0/g' /opt/medusa/config.ini

sed -i "s/^use_failed_downloads =.*/use_failed_downloads = 1/g" /opt/medusa/config.ini
sed -i "s/^delete_failed =.*/delete_failed = 1/g" /opt/medusa/config.ini
...
# Upload
rclone move -c -v --log-file=/home/$username/scripts/logs/uploadACD.${today}.log /home/$username/$local/tv $encrypted:tv
...
#######################
tee "/etc/systemd/system/medusa.service" > /dev/null <<EOF
[Unit]
...
chown -R $username:$username /opt/medusa
chmod +x /home/$username/nzbget/scripts/uploadTV.sh
chown root:root /etc/systemd/system/medusa.service
...
#######################
echo ''
echo "Do you want to allow remote access to Medusa?"
...
select yn in "Yes" "No"; do
    case $yn in
        Yes ) ufw allow 8081; echo ''; echo "Port 8081 open, Medusa is now available over the internet."; echo ''; break;;
